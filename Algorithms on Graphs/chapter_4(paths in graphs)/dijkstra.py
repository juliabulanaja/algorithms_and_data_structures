...
def dijkstra(start, end, adj_list):
    ...
    pq = PriorityQueue()
    pq.put((destinations[start], start))
    ...
    while not pq.empty():
        ...
        for v, w in adj_list[u]:
            if destinations[v] > destinations[u] + w:
                destinations[v] = destinations[u] + w
                pq.put((destinations[v], v))

    return destinations[end] if destinations[end] < float('inf') else -1
